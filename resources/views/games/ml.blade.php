...
@section('title')
<title>Mobile Legends - a Sentimet Analysis App</title>
@endsection
...
<div class="card">
  ...
  <div class="card-body" id="reviewBody">
    ...
    <div class="input-group input-group-sm mb-3">
      <input class="form-control" type="number" value="3" max="5" id="numberReview">
      <div class="input-group-append">
        <button class="btn btn-outline-secondary" type="button" onclick="review()">Refresh</button>
      </div>
    </div>

    <form method="POST" action="{{route('mlbb.saveToTraining')}}">
      @csrf
...
function review()
{
  ...
  $.ajax({
    ...
    success: function(data){
      $('#loader').hide();
      $('#tableallreview').DataTable().destroy();
      $('#reviewBody').css("filter","none");
      $("#tablereview").html(data.msg);
      $('#tableallreview').DataTable().draw();
    }
  });
}
